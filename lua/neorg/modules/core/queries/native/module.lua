...
module.private = {
  ...
  matching_nodes = function(parent, tree, bufnr)
    ...
    local matched_query, how_to_fix = module.private.matching_query(parent, tree.query, { recursive = tree.recursive })
  ...
  predicate_where = function(parent, where, opts)
    ...
    for node in parent:iter_children() do
      ...
      if where[1] == "child_content" then
        ...
        if node:type() == where[2] and vim.split(vim.treesitter.get_node_text(node, temp_buf), "\n")[1] == where[3] then
          return true
        end
      end
    end
