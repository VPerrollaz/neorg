  ...
  delete = function(object, node_type, option)
    ...
    if type(object[option]) == "table" then
      local carryover_tags = vim.tbl_map(function(n)
        local carryover_tag =
          module.required["core.queries.native"].find_parent_node({ n, object.internal.bufnr }, "carryover_tag")
    ...
    local start_row, start_col, end_row, end_col = vim.treesitter.get_node_range(fetched_node)
