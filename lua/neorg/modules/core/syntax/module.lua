  ...
  trigger_highlight_regex_code_block = function(buf, remove, ignore_buf, from, to)
    ...
    for lang_name, curr_table in pairs(lang_table) do
      if curr_table.type == "syntax" then
        ...
        if
          ok == false
          or (ok == true and ((string.sub(result, 1, 1) == ("N" or "V") and count == 0) or (empty_result > 0)))
        then
            ...
            if table.type == "syntax" then
              if lang_name == syntax then
                if empty_result == 0 then
                  ...
                  local _, found_cluster = pcall(vim.api.nvim_exec, string.format("syntax list @%s", group), true)
                  local actual_cluster
                  for match in found_cluster:gmatch(regex) do
                    actual_cluster = match
                  end
                  if actual_cluster ~= nil then
                    module.private.code_block_table[current_buf].loaded_regex[lang_name].cluster = actual_cluster
                  end
                elseif module.private.code_block_table[current_buf].loaded_regex[lang_name].cluster ~= nil then
