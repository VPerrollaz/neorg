...
module.public = {
  ...
  show_capture_popup = function()
    ...
    local selection = module.required["core.ui"].begin_selection(buffer):listener("destroy", { "<Esc>" }, function(self)
      self:destroy()
    end)
...
module.private = {
  ...
  create_project = function(selection, file, task, project)
    ...
    if vim.tbl_isempty(nodes) then
      ...
      selection:destroy()
      module.required["core.gtd.queries"].create("project", project, bufnr, { location, 0 }, false, { newline = false })
      ...
      for i, marker_node in pairs(nodes) do
        ...
        selection:flag(f, extracted_nodes[i]:sub(3), function()
          ...
          local _, _, er, _ = vim.treesitter.get_node_range(marker_node[1])
          module.required["core.gtd.queries"].create("project", project, bufnr, { er, 0 }, false, { newline = false })
    ...
    selection:flag("<CR>", description, {
      callback = function()
        ...
        if generic_lists then
          ...
          local _, sc, er, _ = vim.treesitter.get_node_range(last_list[1])
