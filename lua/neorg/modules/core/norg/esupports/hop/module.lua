...
module.public = {
  ...
  follow_link = function(node, split, parsed_link)
    if node:type() == "anchor_declaration" then
      ...
      local range = module.required["core.integrations.treesitter"].get_node_range(located_anchor_declaration.node)
  ...
  locate_link_target = function(parsed_link_information)
    ...
    return neorg.lib.match(parsed_link_information.link_type)({
      ...
      external_file = function()
        ...
        destination = (vim.tbl_contains({ "/", "~" }, destination:sub(1, 1)) and "" or (vim.fn.expand("%:p:h") .. "/"))
          .. destination
      ...
      _ = function()
        ...
        for id, node in query:iter_captures(document_root, buf_pointer) do
          ...
          if capture == "title" then
            local original_title = module.required["core.integrations.treesitter"].get_node_text(node, buf_pointer)
...
module.private = {
  ...
  write_fixed_link = function(link_node, parsed_link_information, similarities, force_type)
    ...
    local function callback(replace)
      vim.api.nvim_buf_set_text(0, range.row_start, range.column_start, range.row_end, range.column_end, { replace })
    end
